...
<ul>
  ...
  <jsp:useBean id="empSvc" scope="page" class="com.paradisiac.employee.service.EmpService" /><!-- 用scope找沒找到就new -->
  ...
  <li>
     <FORM METHOD="post" ACTION="${pageContext.request.contextPath}/emp.do" >
       ...
       <select size="1" name="empno">
         <c:forEach var="empVO" items="${empSvc.all}" > 
          <option value="${empVO.empno}">${empVO.empName} <%--下拉選單名字去查,但key還是放PK --%>
         </c:forEach>   
       </select>
       <input type="hidden" name="action" value="getOne_For_Display">
       <input type="submit" value="送出">
